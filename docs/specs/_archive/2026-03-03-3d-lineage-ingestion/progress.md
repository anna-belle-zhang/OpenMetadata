...
**Date:** 2026-03-03
**Branch:** main
**Tests:** 55/71 passing (pipeline topology + az-dump subset passing; other sources require generated schemas)
...
| ACI connector | `aci/metadata.py` | ✅ Done |
...
| ado-dump: Dump approval records → approvals.json content verified | `test_writes_approvals_json` |
| ado-dump: Incremental sync (only fetch runs newer than last timestamp) | `test_incremental_fetch_uses_last_timestamp` |
| aci-connector: `run()` calls `get_or_create_service` with `ACI-prd` | `test_run_creates_service_and_pipeline` |
| aci-connector: Task objects carry `image=` and `cpu=` tags | `test_build_pipeline_entity_has_two_tasks` |
| aci-connector: Create ACR `StorageService` named `ACR` | `test_creates_acr_storage_containers_with_latest_tag` |
| aci-connector: Create ACR `StorageContainers` with `latest_tag` | `test_creates_acr_storage_containers_with_latest_tag` |
| aci-connector: Idempotent re-run (no duplicate entities) | `test_idempotent_rerun_updates_tags_without_duplicates` |
| ado-connector: Deploy run has `version=` and `env=` tags | `test_deploy_run_has_version_and_env_tags` |
| stitch-lineage: ACI → Airflow DAG edge is posted | `test_posts_aci_to_airflow_edge` |
| az-dump: Dump ACI state | `test_dumps_all_expected_files` |
| az-dump: ACR tag dumps | `test_dumps_all_expected_files` |
| az-dump: ARM resource dump | `test_dumps_all_expected_files` |
| az-dump: Blob container dump | `test_dumps_all_expected_files` |
| az-dump: Idempotent overwrite | `test_idempotent_overwrites_outputs` |
| az-dump: Fail fast on az error | `test_fail_fast_on_az_error` |
| stitch-lineage: Full cross-dimensional path traversable | `test_posts_full_cross_dimension_path` |

**❌ Still missing from Stage 1 (0/26)**
