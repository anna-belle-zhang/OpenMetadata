...
THEN the DAG entity's `image_build_ref` custom property is set to the FQN of the most recent image build Pipeline whose date is on or before the DAG run's `endDate`

### Enrich DAG entity with active infra deploy ref
GIVEN ADO Pipeline entities exist in OM with `pipeline_type="infra_deploy"`
AND an Airflow DAG entity exists with a `PipelineStatus.endDate`
WHEN the DAG enricher runs
THEN the DAG entity's `infra_deploy_ref` custom property is set to the FQN of the most recent infra deploy Pipeline whose date is on or before the DAG run's `endDate`
